	...
	**/
	...
	public function get_near_shop($lat,$lang)
	{
        ...
        for ($i = 1; $i < $total_restaurant; $i++) {
        	...
        	if ($this->distance($poslat, $poslng, $location[$i]['lat'], $location[$i]['lng'], "K") <= 50) {
	...
	**/
	...
	public function updates(){
		...
		if(isset($_GET)){
			...
			$update = $this->admin_m->update($data,$id,$table);
			if($update){
				echo json_encode(array('st'=>1));
			}else{
				echo json_encode(array('st'=>0));
	...
	function empty(){
		is_test();
		$array = ['settings','users','about','about_content','allergens','dine_in','items','item_content','item_extras','item_packages','item_sizes','offline_payment','menu_type','order_item_list','order_payment_info','order_user_list','pages','payment_info','permission_list','pickup_points_area','portfolio','pricing','privacy','profile_home','reservation_date','	restaurant_list','staff_list','table_areas','table_list','terms','users_active_features','users_active_order_types','users_rating','user_settings','delivery_area_list'];
