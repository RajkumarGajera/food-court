...
public function user_login()
{
	...
	if ($this->form_validation->run() == FALSE) {
		...
		$is_staff = $this->admin_m->check_staff($email);
		if($query['result']==0 && $is_staff==1){
			$is_staff = $this->admin_m->check_staff_login_info($email,$password);
			if($is_staff['result']==0 || $is_staff['result']==1):
				$msg = '<div class="alert alert-danger alert-dismissible">
				<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
				...
				foreach($query['result'] as $row):
					...
					if($row->is_active == 1):
						...
						if($row->user_role==1):
							...
							//add features
							$this->insert_features($row->id);

							//add order_types
							$this->add_order_types($row->id);

							//add qr code
							$this->qr_code($row->username);
...
public function user_registration($slug=''){
	...
	$this->form_validation->set_rules('email', 'Email', 'trim|required|xss_clean|valid_email|is_unique[staff_list.email]',array('is_unique'=>'The email is already Exists in staff list'));	
	...
	if ($this->form_validation->run() == FALSE) {
		...
		if($username_check==1){
			$msg = '<div class="alert alert-danger alert-dismissible">
			<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
			<strong><i class="fas fa-frown"></i> Sorry ! </strong> This name <b>'.str_slug($this->input->post('username',TRUE)).'</b> already exists!!
			</div>';
			echo json_encode(array('st' => 0, 'msg'=> $msg,));
			exit();
		}else{
			...
			if($type=='monthly' || $type=='yearly'){
				...
				$this->settings['is_email_verification']==1?$is_mail=1:$is_mail=0;
				$is_payment=0;
				$this->settings['auto_approval']==1?$approve = 1:$approve=0;


			}else{
				if($this->settings['is_auto_verified']==1):
					$verify = 1;
					$is_payment = 1;
					$is_mail = 0;
					$approve = 1;
				else:
					$this->settings['is_email_verify_free']==1?$verify=0:$verify=1;
					$this->settings['is_email_verify_free']==1?$is_payment=0:$is_payment=1;
					$this->settings['is_email_verify_free']==1?$is_mail=1:$is_mail=0;
					$approve = 1;
				endif;
			};
			...
			if(is_demo() == 1):
				...
				$msg = '<div class="alert alert-success alert-dismissible">
						<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
						<p><strong><i class="far fa-smile"></i> Congratulations! </strong> Account Create Successfully</p>
			...
			if($insert):
				...
				if(isset($is_mail) && $is_mail==1):
					$msg = '<div class="alert alert-success alert-dismissible">
					<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
					<p><strong><i class="far fa-smile"></i> Congratulations ! </strong>Account Create Successfully</p>
					<p>Account confirmation link has been emailed to you, follow the link in the email to continue.</p>
					</div>';
...
public function verify($username){
	...
	if($u_info['is_verify']==1):
		...
		if($u_info['package_type']=='free' || $u_info['package_type']=='trial' || $u_info['package_type']=='weekly' || $u_info['package_type']=='fifteen' || $u_info['account_type'] == 0):
...
public function create_admin(){
	...
	public function check_vaild($code,$site_id){
		...
		$result = '{"st":1,"msg":"Success","active_code":"GPLLICENSE","li":"526f0a46f67201715d149a0c764cee86","license_name":"extend","active_key":"1","sold_at":"23/02/2222","li_code":"526f0a46f67201715d149a0c764cee86"}';
		// curl_close($ch);
