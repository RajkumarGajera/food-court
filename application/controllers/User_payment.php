...
----------------------------------------------*/
    ...
    public function order_info($insert,$info){
        ...
        $data['delivery_area'] = $info['delivery_area'];
        if($order_type==4){
        ...
        if(shop($shop_id)->is_whatsapp ==1):
            ...
            if($info['order_type']==1 || $info['order_type']==4 || $info['order_type']==5):
                if($data['shop_info']['delivery_charge_in'] !=0){
                    $data['net_price'] = $info['total'];
                    $data['delivery_charge'] = $data['shop_info']['delivery_charge_in'].' '.shop($shop_id)->icon;
                }else{
                    $data['net_price'] = $info['total'];
                    $data['delivery_charge'] = lang('free');
        ...
        return ['st'=>1,'msg'=>$msg,'shop_id'=>$shop_id,'order_id'=>$order_id,'load_data'=>$load_data,'qrlink'=>$qrLink,'track_link'=>$track_link];
