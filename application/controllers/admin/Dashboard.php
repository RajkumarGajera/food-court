...
public function add_order_types(){
	...
	**/ 
	public function change_packege_by_admin($id)
	{
		...
		$data = array(
			...
			'start_date'=>d_time(),
			'end_date'=>add_year($package_info['package_type']),
			'is_payment'=>1,
			'is_expired'=>0,
		);	
	...
	**/
	public function verify_by_admin($username,$status){
		...
		$data = array('is_verify'=>$status,'verify_time'=>d_time(),'is_payment'=>1,'is_expired'=>0,'is_request'=>0,'start_date'=>d_time(),'end_date'=>add_year($u_info['package_type']));
	...
	**/
	public function payment_by_admin($username,$status){
		...
		$data = array('is_payment'=>1,'is_expired'=>0,'is_request'=>0,'start_date'=>d_time(),'end_date'=>add_year($user_info['package_type']),'is_request'=>0);
	...
	**/ 
    ...
    public function create_page(){
		...
		$this->form_validation->set_rules('title', 'Title', 'trim|required|xss_clean');
		if($id==0):
		$this->form_validation->set_rules('slug', 'Slug', 'trim|required|xss_clean|is_unique[pages.slug]');
		else:
	...
	public function new_user(){
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data = array(
				...
				'start_date' => d_time(),
				'verify_time' => d_time(),
				'end_date' => add_year($package['package_type']),
				'is_active' => 1,
				'is_verify' => 1,
				'is_payment' => 1,
			);
...
public function apply_upgrade(){
	...
	public function check_vaild($data){
		...
		$ch = curl_init(URL."site_api/upgrade_license/");  
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->update();
				...
				if($check_valid->st ==1):
					...
					redirect(base_url('admin/dashboard/check_valid_purchases'));
				endif;	
		}
	}
