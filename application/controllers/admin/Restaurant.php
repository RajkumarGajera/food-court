...
public function add_order_configuration(){
	is_test();
	$this->form_validation->set_rules('is_payment[]', 'Enable Payment', 'trim|required|xss_clean');
	if ($this->form_validation->run() == FALSE) {
		$this->session->set_flashdata('error', validation_errors());
...
public function order_status($id,$status)
{
	...
	if($status==1){
		...
		if(isset($sms->is_accept_sms) && $sms->is_accept_sms==1):
			...
			$this->sendSMS($data);
			$this->session->set_flashdata('success', !empty(lang('sms_sent'))?lang('sms_sent'):'Message Sent');
		endif;
...
public function order_status_by_ajax($id,$status)
{
	...
	if($status==1){
		...
		if(isset($sms->is_accept_sms) && $sms->is_accept_sms==1):
			$data = ['phone' => $phone, 'text' => create_msg($replace_data,$accept_message)];
 			$this->sendSMS($data);
 			$msg = !empty(lang('sms_sent'))?lang('sms_sent'):'Message Sent';
		endif;
...
public function add_configuration()
{
	...
	$this->form_validation->set_rules('tax_fee', 'Tax Fee', 'trim|xss_clean|callback_number_check');
...
public function add_menu_style(){
 	is_test();
    	$this->form_validation->set_rules('menu_style', 'Menu Style', 'trim|required|xss_clean');
    	if ($this->form_validation->run() == FALSE) {
    		$this->session->set_flashdata('error', validation_errors());
    		redirect(base_url('admin/restaurant/profile'));
    	}else{	
...
public function statistics()
{
	...
	$data['main_content'] = $this->load->view('backend/restaurant/statistics', $data, TRUE);
	$this->load->view('backend/index',$data);
}

public function add_pickup_slots($shop_id){
	is_test();
	...
	if ($this->form_validation->run() == FALSE) {
		...
		redirect(base_url('admin/restaurant/pickup_points/'.$shop_id));
		   	...
		   	$id = $this->input->post('id');
			   $data = array(
			   	'pickup_time_slots' => json_encode($this->input->post('time_slots')),
			   );
...
public function add_order_item($uid,$id)
{
	...
	$total_price = grand_total($price_format,$order_info['delivery_charge'],$order_info['discount'],$order_info['tax_fee'],$order_info['order_type']);
	$data = array(
...
public function whatsapp(){
	$shop_info = $this->common_m->get_restaurant_info_by_id(1);
	$order_list = $this->common_m->get_order_item('202',$shop_info['shop_id']);
	$data = '';
	$i=1; 
	...
	$data.= lang('discount')."\t".$discount."\n";
	$data.= lang('tax')."\t".$tax_fee."\n";
	if($order_type !=4):
	$data.= lang('shipping')."\t".$delivery_charge."\n";
	endif;
	...
	if(isset($pickup_time) && !empty($pickup_time)):
		$data .= "---------------------------------------\n";
		$data.= lang('pickup_time')."\t : ".urlencode($pickup_time)."\n";
	endif;
	...
	if(isset($delivery_area) && !empty($delivery_area)):
		$data.= "---------------------------------------\n";
		$data.= lang('gmap_link')."\t : ".$delivery_area."\n";
	endif;
	...
	$data.= urlencode(base_url('my-orders/'.$shop_info['username'].'?phone='.$phone.'&orderId='.$order_id));
	...
	echo "<pre>";print_r(urlencode($msg));exit();
	exit();
