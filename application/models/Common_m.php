...
public function count_total_user()
{
	...
	**/
    ...
    $this->db->where('or.customer_rating !=','');
		$query = $this->db->get();
		$query = $query->result_array();
		return $query;
	}
	...
	**/
    ...
    public function get_all_menu_packages($id,$limit=0)
    {
        ...
        $query = $query->result_array();
        foreach($query as $key => $value){
            $this->db->select('i.id as items_id,i.title,i.thumb as item_thumb,i.cat_id,i.allergen_id,i.veg_type,i.price as item_price,i.overview,a.name as allergen');
            $this->db->from('items i');
            $this->db->join('allergens a','a.id = i.allergen_id','LEFT');
    ...
    public function get_item_by_cat_id($user_id,$cat_id,$limit=0,$per_page,$offset,$total)
    {
    ...
    public function get_single_qr_menu_by_id($id)
    {
        ...
        $query = $query->result_array();
        foreach($query as $key => $value){
            $this->db->select('i.id as items_id,i.title,i.thumb as item_thumb,i.cat_id,i.allergen_id,i.veg_type,i.price as item_price,a.name as allergen,i.overview');
            $this->db->from('items i');
            $this->db->join('allergens a','a.id = i.allergen_id','LEFT');
            $this->db->where_in('i.id',json_decode($value['item_id']));
