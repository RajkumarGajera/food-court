...
<script src="https://maps.googleapis.com/maps/api/js?key=<?=  $shop->gmap_key;?>" type="text/javascript"></script>
<?php endif;?>
<div class="row">
	<?php foreach ($item_list as $key => $order): ?>
					...
					<div class="box-tools pull-right">
						<?php if($order['order_type']!=7): ?>
							<div class="row ">
									...
									<?php else: ?>
										<a class="btn btn-info" href="<?= base_url("admin/restaurant/get_item_list_by_order_id/{$order_details['uid']}?action=Edit"); ?>">
						...
						<div class="order_information pt-5">
							<div class="orderTop">
								<div class="d_flex_alert-admin">
									...
									<div class="double_text">
										<div class="text-left">
											<h5><label class="label bg-primary-soft"> <?= lang('order_type'); ?> : <?= order_type($order['order_type']);?></label></h5>
											...
											<?php if($order['order_type']==4): ?>
												<div class="mt-15">
													<label class="label default-light-active  p-5 fz-14" data-toggle="tooltip" title="Pickup Time"><?= lang('pickup_time'); ?> :  <?=!empty($order['pickup_time'])?$order['pickup_time']: time_format_12($order['reservation_date']) ;?></label> &nbsp;
							...
							<div class="mt-20">
								<?php if($order['status']==0): ?>
									...
									<a href="#deliveryModal" data-toggle="modal" class="btn success-light-active hidden" data-toggle="tooltip" title="Mark as Completed"><i class="icofont-hand-drag1"></i> &nbsp; <?= !empty(lang('deliverey'))?lang('deliverey'):"Delivery"; ?> </a> &nbsp;
								...
								<?php elseif($order['status']==2): ?>
									...
									<?php if($order['is_delivery']==1): ?>
										<a href="javascript:;" class="btn btn-secondary" data-toggle="tooltip" title="Completed"><i class="fa fa-check"></i> &nbsp; delivered </a> &nbsp;
									<?php endif;?>
							...
							<table class="table table-bordered table-condensed table-striped">
								...
								<?php foreach ($order['item_list'] as $key => $row): ?>
									...
									<?php 
										...
										$grand_total = number_formats(grand_total($total_price,$order['delivery_charge'],$order['discount'],$order['tax_fee'],$order['coupon_percent'],$order['tips'],$order['order_type']),restaurant()->id);
								...
								<tr class="bolder text-left" >
									...
									<td colspan="2">
										<div class="bottomPrice">
											...
											<p class="flex-between"><span><?= lang('discount'); ?> </span> <b><?= currency_position(get_percent($price,$order['discount']),restaurant()->id) ;?> </b></p>
											<?php endif;?>
		...
		<div class="col-md-4">
			...
			<div class="row">
			 ...
			 <?php $coordinates = getCoordinatesAttribute($order['delivery_area']);?>
			<?php endif;?>
									...
									<?php if($order['is_db_completed']==1): ?>
										<li>
											...
											<div class="timeline-item">
												...
												<div class="timeline-body">
													<h5><?= !empty(lang('status_from'))?lang('status_from'):"Status from" ;?>: <?= lang('delivery_staff'); ?></h5>
													<p><?= !empty(lang('delivered_by'))?lang('delivered_by'):"Delivered By" ;?> : <b><?= $dboy_info['name'] ;?></b></p>
												</div>
											</div>
										</li>
									<?php endif; ?>
...
<?php if(isset($shop->is_gmap) && $shop->is_gmap ==1 && !empty($shop->gmap_key) && isset($coordinates)): ?>

<script type="text/javascript">
	function initialize() {
	   var latlng = new google.maps.LatLng(<?= isset($coordinates['longitude'])?$coordinates['longitude']:'' ;?>,<?= isset($coordinates['latitude'])?$coordinates['latitude']:'' ;?>);
