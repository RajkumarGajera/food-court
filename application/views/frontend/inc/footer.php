	...
	<?php if(isset($id)): ?>
	 ...
	 <?php foreach ($days as $key => $day): 
 		...
 		endforeach ?>
 			...
 			<?php if(!empty($time)): ?>
	 			<?php  $now = strtotime(get_time());?>
	 			<?php $start_time = isset($time['start_time'])? strtotime($time['start_time']):'';?>
	 			<?php 
	 				$end_time =  isset($time['end_time'])?strtotime($time['end_time']):"";
	 				$end_time = $end_time <= $start_time ? $end_time + 2400 : $end_time;
	 			?>

	 			<div class="close_time" data-id="<?=  $shop->id;?>" data-status='<?= $now >= $start_time  || $now <= $end_time?0:1; ;?>'></div>  
