						...
						<table class="table table-bordered table-condensed table-striped" id="example1">
							...
							<?php foreach ($order['item_list'] as $key => $row): ?>
								...
								<?php 
									...
									$grand_total = number_format(grand_total($total_price,$order['delivery_charge'],$order['discount'],$order['tax_fee'],$order['coupon_percent'],$order['tips'],$order['order_type']),2);
								?>
							<?php endforeach ?>
							<tr class="bolder" >
								<td colspan="3"></td >
								<td colspan="2">
									<div class="bottomPrice">
											...
											<?php if($order['discount'] !=0): ?>
												<p class="flex-between"><span><?= lang('discount'); ?> </span> <b><?=currency_position(get_percent($price,$order['discount']),$order['shop_id']) ;?></b></p>
											<?php endif;?>
