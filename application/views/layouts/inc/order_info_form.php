	  		...
	  		<div class="form-group col-md-6">
	  			...
	  			<select name="order_type" class="form-control order_type">
	  				<option value=""><?= !empty(lang('select_order_type'))?lang('select_order_type'):'select order type' ;?></option>
		  				...
		  				<?php else: ?>
		  					<option value="<?=  $types['type_id'];?>"  data-pay="<?= $types['is_payment'];?>" data-slug="<?=  $types['slug'];?>"><?=  $types['type_name'];?></option>
		  				<?php endif; ?>
		  			<?php endforeach ?>
	  			</select>
	  		</div>
	...
	<div class="step_2">
		...
		<div class="dis_none show_address">
			...
			<div class="row">
				...
				<div class="form-group col-md-12">
					<input type="text" name="delivery_area" class="form-control gmap_link" value="<?= !empty(auth('gmap_link'))?auth('gmap_link'):"" ;?>" placeholder="<?= !empty(lang('google_map_link'))?lang('google_map_link'):'Google map link' ;?>">
				</div>
			</div>
		</div><!-- show_address -->
		...
		<div class="couponArea" style="display:none;">
			...
			<div class="couponField" style="display:none;">
				<div class="couponInput-group">
					...
					<input type="text" name="coupon_code" class="form-control coupon_code">
		...
		<?php if(is_feature($shop_info['user_id'],'online-payment')==1 && is_active($shop_info['user_id'],'online-payment') && overlay==1): ?>		
			<div class="makePayment <?= is_package;?>" style="display:none;">
				<div class="form-group">
					<div class="">
						<label class="custom-radio-2 f-color mr-5"><input type="radio" name="use_payment" value="1"> <b><?= lang('pay_now');?></b></label> &nbsp;
						<label class="custom-radio-2 f-color"><input type="radio" name="use_payment" value="0" checked> <b><?= lang('pay_later');?></b></label>
					</div>
				</div>
			</div>
		<?php endif ?>
